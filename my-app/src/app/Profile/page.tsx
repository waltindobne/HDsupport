...
export default function Profile() {  
    ...
    const profileDados = [
      {id:1, nome: "Sérgio nauffal", cargo: "Chefe", email: "sergionaufall@gmail.com", numero: "41 9999999"}
    ]
  ...
  <div className="justify-center items-center flex">  
    <div className="border-2 rounded-[20px] mt-10 w-4/5">  
      <div className="flex ml-8 items-center w-[638px] h-[136px] border-slate-50">  
        ...
        <div className="ml-2">  
         {profileDados.map(profile => (
           <div>
            <h1 className="font-bold text-white dark:text-black">{profile.nome}</h1>  
           <h3 className="text-[#2F76FF] font-bold">{profile.cargo}</h3>  
           <h3 className="text-white dark:text-[#8C8C8C] ">{profile.email}</h3>  
           </div>
         )) }
        </div>  
      </div> 
    ...
    <div className="flex justify-center items-center mt-10">  
      <div className="border-2 w-4/5 h-[300px] rounded-[20px]">  
        <div className="flex justify-between">  
          <div className="font-bold text-2xl  text-white p-8 dark:text-black">Informações</div>  
          <div className="p-8 font-bold flex text-xl"> <button className='flex'>   <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className=" text-white lucide lucide-file-pen-line dark:text-black"><path d="m18 5-3-3H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2"/><path d="M8 18h1"/><path d="M18.4 9.6a2 2 0 1 1 3 3L17 17l-4 1 1-4Z"/></svg> <h1 className='ml-2  text-white dark:text-black'>Editar</h1></button></div>  
        </div>  
        {profileDados.map(profile => (
          <div className="flex flex-col ml-8">  
          <h1 className="text-[#2F76FF] font-bold">Nome</h1>  
          <p className="font-bold  text-white dark:text-black ">{profile.nome}</p>  
  
          <h1 className="text-[#2F76FF] font-bold mt-3">E-mail</h1>  
          <p className="font-bold  text-white dark:text-black  ">{profile.email}</p>  
  
          <h1 className="text-[#2F76FF] font-bold mt-3">Telefone</h1>  
          <p className="font-bold  text-white dark:text-black ">{profile.numero}</p>  
        </div>  
        ))}
      </div>  
    </div>  
