...
const Equipamento = () => {
	...
	localStorage.setItem('token', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1bmlxdWVfbmFtZSI6InByb2dyYW1hQGVtcGxveWVyLmNvbS5iciIsInJvbGUiOiJSSCIsIm5iZiI6MTcxNDQ5ODM4MCwiZXhwIjoxNzE0NTI3MTgwLCJpYXQiOjE3MTQ0OTgzODB9.l3GonQmxqWUWUdIiVeWD9_lYlbYS605innB0Zl-Dtuo');
    ...
    const handleSubmitAdd = async (event) => {
        ...
        try {
            const response = await axios.post('https://hd-support-api.azurewebsites.net/api/Equipamentos/Registro-Equipamentos', newEquipamento);
					...
					<tbody className="">
						{currentItems.map((equipamento, index) => (
							<tr key={index} className="h-[50px] border-y">
								...
								<td className="text-center">{statusEquipamento[equipamento.statusEquipamento]}</td>
					...
					<div className="fixed inset-0 flex items-center justify-center  bg-opacity-50">
						<div className="bg-neutral-800 p-4 pt-6 px-6 rounded-[8px] w-[400px] ">
							...
							<div className="mb-4 mt-4">
								<input
								type="text"
								value={newEquipamento.profissional_HD}
								onChange={handleInputChangeAdd}
								name="profissional_HD"
								placeholder='Profissional HD'
								className="mt-1 w-full bg-neutral-700 p-1 h-11 rounded-[8px] px-4"
								/>
							</div>
						...
						<div className=" p-4 px-6 rounded-[8px] w-[600px] bg-neutral-800 ">
							...
							<table className='w-full'>
                                <tbody>
                                    ...
                                    <tr><th className="text-center mt-5 border-b border-neutral-700 py-3">Status Equipamento</th><td className="text-center mb-5 border-b border-neutral-700 py-3">{statusEquipamento[selectedEquipamento.statusEquipamento]}</td> </tr>
                                </tbody>
                            </table>
						</div>
					</div>
